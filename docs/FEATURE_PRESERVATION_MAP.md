...
|---|---|---|
| Payment gateway credentials | `PaymentGateway` (resolved through `PaymentCollection`) | **No, and now said out loud.** One platform account collects for every shop. Decision W1 (2026-09-05) already settles the rule — a merchant's product proceeds belong to that merchant, and GP-STORE does not pool everybody's money in one account and redistribute it — so today's behaviour is a **gap against a recorded decision**, not an open question. §17 forbids inventing per-merchant accounts on an architectural preference, so `PaymentCollection` is the boundary and it names no provider: the question is asked per shop, the merchant is told the answer, the non-conformance is logged at startup, and configuring `MERCHANT_COLLECTS` without an implementation fails at boot rather than misreporting every settlement. What is still open is the provider, merchant onboarding and KYC, the settlement mechanism, fees and refund funding — the boundary stays provider-agnostic until those are decided. |
