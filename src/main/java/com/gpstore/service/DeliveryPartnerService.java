...
public class DeliveryPartnerService {
    ...
    public DeliveryPartner save(DeliveryPartner partner) {
        ...
        if (isNewPartner && saved.getAccount() == null) {
            // saved gets reassigned below (saved = repository.save(saved)),
            // which makes it NOT effectively final - Java doesn't allow a
            // lambda to capture a local variable that's reassigned
            // anywhere in its enclosing scope, even if the reassignment
            // happens after the lambda itself. savedPartner exists purely
            // so the lambda has something legal to close over.
            final DeliveryPartner savedPartner = saved;

            Customer account = customerRepository.findByMobileNumber(savedPartner.getMobile())
                    .orElseGet(() -> {
                        Customer newAccount = new Customer();
                        newAccount.setFullName(savedPartner.getName());
                        newAccount.setMobileNumber(savedPartner.getMobile());
