...
- **Self-service account deletion**, both required paths: an in-app
  "Delete Account" option (Profile screen) and a public web page
  (`web/account-deletion.html`, deployed automatically alongside the
  Flutter web build) that works without the app installed. See
  `CustomerService.deleteOwnAccount`'s doc comment on the backend for
  exactly what gets deleted outright versus anonymized (order/invoice
  history is retained for tax reasons, not deleted - this is standard
  practice and still satisfies the policy). This was a genuine gap when
  this checklist was first written; Google Play rejects new submissions
  outright without it, confirmed against their current policy - not
  optional the way this document originally implied.
